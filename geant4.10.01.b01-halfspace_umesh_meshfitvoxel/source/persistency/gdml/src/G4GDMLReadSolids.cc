...
// ********************************************************************
//
// $Id: G4GDMLReadSolids.cc 80934 2014-05-16 14:06:39Z gcosmo $
//
...
void G4GDMLReadSolids::SolidsRead(const xercesc::DOMElement* const solidsElement)
{
        ...
        iter != 0; iter = iter->getNextSibling())
   {
      ...
      //qiu
           else if (tag=="HalfSpaceSolid") { HalfSpaceSolidRead(child); }
           else if (tag=="HalfSpaceBoolean") { HalfSpaceBooleanRead(child); }
      else
      {
...
//qiu start adding code here


//qiu
G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpacePlaneRead(const xercesc::DOMElement* const Element)
{
    G4double A=0.,B=0., C=0., D=0.;
    G4String theAxis;
    G4int    theSense =1; //default value
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (!attribute)
       {
         G4Exception("G4GDMLReadDefine::Read()",
                     "InvalidRead", FatalException, "No attribute found!");
         return NULL ;
       }
       const G4String attName = Transcode(attribute->getName());
       ...
       if (attName=="Sense") {theSense  = eval.EvaluateInteger(attValue); } else
       if (attName=="Axis") { theAxis = attValue; } else
       if (attName=="A") { A = eval.Evaluate(attValue); } else
       if (attName=="B") { B = eval.Evaluate(attValue); } else
       if (attName=="C") { C = eval.Evaluate(attValue); } else
       if (attName=="D") { D = eval.Evaluate(attValue); }
    }

    //making a plane
    if (theSense != -1 && theSense != 1) {
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "The Sense should be 1 or -1! We set it as 1.");
        theSense = 1;
    }
    if (theAxis == "X") { A=1.; B=0.; C=0.; } else
    if (theAxis == "Y") { A=0.; B=1.; C=0.; } else
    if (theAxis == "Z") { A=0.; B=0.; C=1.; } else
    {
        if (A==0. && B==0. && C==0.) {
            G4Exception("G4GDMLReadDefine::Read()",
                        "InvalidRead", FatalException, "The A/B/C value should not be 0 at the same time!");
            return NULL;
        }
    }
    return new G4HalfSpacePlane(A, B, C, D*m_lunit, theSense);
}

G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpaceSphereRead(const xercesc::DOMElement* const Element)
{
    G4double CenterX=0.,CenterY=0., CenterZ=0., Radius=0.;
    G4int    theSense =1; //default value
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
    XMLSize_t attributeCount = attributes->getLength();
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="Sense") {theSense  = eval.EvaluateInteger(attValue); } else
       if (attName=="X") { CenterX = eval.Evaluate(attValue); } else
       if (attName=="Y") { CenterY = eval.Evaluate(attValue); } else
       if (attName=="Z") { CenterZ = eval.Evaluate(attValue); } else
       if (attName=="Radius") { Radius = eval.Evaluate(attValue); }
    }
    ...
    if (Radius <= 0.) {
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "The radius should greater than 0.");
        return NULL;
    }
    return new G4HalfSpaceSphere(CenterX*m_lunit, CenterY*m_lunit, CenterZ*m_lunit, Radius*m_lunit, theSense);
}

G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpaceCylinderRead(const xercesc::DOMElement* const Element)
{
    G4double Center1=0.,Center2=0., Radius=0.;
    G4String theAxis;
    G4int    theSense =1; //default value
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="Sense") {theSense  = eval.EvaluateInteger(attValue); } else
       if (attName=="Axis") { theAxis = attValue; } else
       if (attName=="Center1") { Center1 = eval.Evaluate(attValue); } else
       if (attName=="Center2") { Center2 = eval.Evaluate(attValue); } else
       if (attName=="Radius") { Radius = eval.Evaluate(attValue); }
    }
    //checking
    if (theSense != -1 && theSense != 1) {
    ...
    G4HSAxis aAxis ;
    if (theAxis == "X") { aAxis = AxisX;  } else
    if (theAxis == "Y") { aAxis = AxisY;  } else
    if (theAxis == "Z") { aAxis = AxisZ;  } else
    {
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "Unrecognized Axis!");
        return NULL;
    }
    return new G4HalfSpaceCylinder(aAxis, Radius*m_lunit, Center1*m_lunit, Center2*m_lunit, theSense);

}

G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpaceConeRead(const xercesc::DOMElement* const Element)
{
    G4double X=0.,Y=0., Z=0., T2=0.;
    G4String theAxis;
    G4int    theSense =1; //default value
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="Y") { Y = eval.Evaluate(attValue); } else
       if (attName=="Z") { Z = eval.Evaluate(attValue); } else
       if (attName=="T2") { T2 = eval.Evaluate(attValue); }
    }
    ...
    if (T2 <= 0.) {
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "The T2 should greater than 0.");
        return NULL;
    }
    G4HSAxis aAxis ;
    if (theAxis == "X") { aAxis = AxisX;  } else
    ...
    //T2 is no-unit quantity, therefore do not need to multiply with unit
    return new G4HalfSpaceCone(aAxis, T2, X*m_lunit, Y*m_lunit, Z*m_lunit, theSense);
}

G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpaceQuadricRead(const xercesc::DOMElement* const Element)
{
    G4double A=0., B=0., C=0., D=0., E=0., F=0., G=0., H=0., I=0., J=0.;
    G4int    theSense =1; //default value
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="B") { B = eval.Evaluate(attValue); } else
       if (attName=="C") { C = eval.Evaluate(attValue); } else
       if (attName=="D") { D = eval.Evaluate(attValue); } else
       if (attName=="E") { E = eval.Evaluate(attValue); } else
       if (attName=="F") { F = eval.Evaluate(attValue); } else
       if (attName=="G") { G = eval.Evaluate(attValue); } else
       if (attName=="H") { H = eval.Evaluate(attValue); } else
       if (attName=="I") { I = eval.Evaluate(attValue); } else
       if (attName=="J") { J = eval.Evaluate(attValue); }
    }
    //checking
    if (theSense != -1 && theSense != 1) {
        G4Exception("G4GDMLReadDefine::Read()",
    ...
    //When unit change from mm->cm, A,B,C,E,F,H keep the same, D,G,I should *0.1, J should *0.01
    return new G4HalfSpaceQuadric(A, B, C, D*m_lunit, E, F, G*m_lunit, H, I*m_lunit, J*m_lunit*m_lunit, theSense);

}

G4HalfSpaceSurface * G4GDMLReadSolids::HalfSpaceTorusRead(const xercesc::DOMElement* const Element)
{
    G4double CenterX=0.,CenterY=0., CenterZ=0.;
    G4double AxsX=0.,AxsY=0., AxsZ=0.;
    G4double MaxRadius =0., MinRadius1 =0., MinRadius2=0.;
    G4int    theSense =1; //default value
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="Sense") {theSense  = eval.EvaluateInteger(attValue); } else
       if (attName=="CenterX") { CenterX = eval.Evaluate(attValue); } else
       if (attName=="CenterY") { CenterY = eval.Evaluate(attValue); } else
       if (attName=="CenterZ") { CenterZ = eval.Evaluate(attValue); } else
       if (attName=="AxisX") { AxsX = eval.Evaluate(attValue); } else
       if (attName=="AxisY") { AxsY = eval.Evaluate(attValue); } else
       if (attName=="AxisZ") { AxsZ = eval.Evaluate(attValue); } else
       if (attName=="MaxRadius") { MaxRadius = eval.Evaluate(attValue); } else
       if (attName=="MinRadius1") { MinRadius1 = eval.Evaluate(attValue); } else
       if (attName=="MinRadius2") { MinRadius2 = eval.Evaluate(attValue); }
    }
    ...
    if (MaxRadius <= 0. || MinRadius1 <= 0.){
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "The radius should greater than 0.");
        return NULL;
    }

    if (AxsX==0. && AxsY==0. && AxsZ==0.) {
        G4Exception("G4GDMLReadDefine::Read()",
                    "InvalidRead", FatalException, "The Axis should not be 0,0,0 !");
        return NULL;
    }
    if (MinRadius2 <= 0.) MinRadius2 = MinRadius1; //circular torus

    G4ThreeVector aCenter (CenterX*m_lunit, CenterY*m_lunit, CenterZ*m_lunit);
    G4ThreeVector aAxis (AxsX, AxsY, AxsZ);

    return new G4HalfSpaceTorus(aCenter, aAxis, MaxRadius*m_lunit, MinRadius1*m_lunit, MinRadius2*m_lunit, theSense);

}

std::vector <G4HalfSpaceSurface * >  G4GDMLReadSolids::HalfSpaceSurfacesRead(const xercesc::DOMElement* const Element)
{
    std::vector <G4HalfSpaceSurface * > aSurfaceList;
         ...
         iter != 0;iter = iter->getNextSibling())
    {
       ...
       if (tag=="Plane")    { aSurfaceList.push_back(HalfSpacePlaneRead(child) ); } else
       if (tag=="Sphere")   { aSurfaceList.push_back(HalfSpaceSphereRead(child) ); }   else
       if (tag=="Cylinder") { aSurfaceList.push_back(HalfSpaceCylinderRead(child) ); } else
       if (tag=="Cone")     { aSurfaceList.push_back(HalfSpaceConeRead(child) ); } else
       if (tag=="Quadric")  { aSurfaceList.push_back(HalfSpaceQuadricRead(child) ); }    else
       if (tag=="Torus")    { aSurfaceList.push_back(HalfSpaceTorusRead(child) ); }
       else
       {
...
G4BoundingBox3D *    G4GDMLReadSolids::BoundaryBoxRead(const xercesc::DOMElement* const Element)
{
    G4ThreeVector LowerPoint, UpperPoint;
         ...
         iter != 0;iter = iter->getNextSibling())
    {
       ...
       if (!child)
       {
         G4Exception("G4GDMLRead::BoundaryBoxRead()", "InvalidRead",
                     FatalException, "No child found!");
         return NULL;
       }
       const G4String tag = Transcode(child->getTagName());

       if (tag=="LowerPoint") {  LowerPoint = NodeRead(child) ;    } else
       if (tag=="UpperPoint") {  UpperPoint = NodeRead(child) ;    }
       else
       {
         G4String error_msg = "Unknown tag in define: "+tag;
         G4Exception("G4GDMLReadDefine::BoundaryBoxRead()", "ReadError",
                     FatalException, error_msg);
       }
    }

    return new G4BoundingBox3D(LowerPoint*m_lunit, UpperPoint*m_lunit);
}

G4double            G4GDMLReadSolids:: VolumeSizeRead(const xercesc::DOMElement* const Element)
{
    G4double aValue =0.;  //default volume value
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
    XMLSize_t attributeCount = attributes->getLength();
    for (XMLSize_t attribute_index=0;
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="value") { aValue = eval.Evaluate(attValue); }
    }
    return aValue*m_lunit*m_lunit*m_lunit;
}

G4double            G4GDMLReadSolids::SurfaceAreaRead(const xercesc::DOMElement* const Element)
{
    G4double aValue =0.;  //default volume value
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
    ...
    return aValue*m_lunit*m_lunit;
}

G4Polyhedron *  G4GDMLReadSolids::VisPolyhedronRead(const xercesc::DOMElement* const Element )
{
    G4String aRef;
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
         ...
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="value") { aRef = attValue; }
    }

    return GetPolyhedron(aRef);
}


void   G4GDMLReadSolids::HalfSpaceSolidRead(const xercesc::DOMElement* const Element)
{

    std::vector <G4HalfSpaceSurface * > aSurfaceList;
    G4BoundingBox3D *   aBoundaryBox;
    G4double aVolumeSize =0., aSurfaceArea =0.;
    G4Polyhedron * aVisPolyhedron;
    G4String  aName;
    G4double lunit = 1.0;

    //first read the name and unit
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
    XMLSize_t attributeCount = attributes->getLength();
    for (XMLSize_t attribute_index=0;
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="name") { aName = attValue;} else
       if (attName == "lunit"){ lunit = eval.Evaluate(attValue) ;}

    }

    if (aName.empty())        {
        G4Exception("G4GDMLReadSolid::Read()",
                    "InvalidRead", FatalException, "name should not be empty!");
        return;
    }
    m_lunit = lunit; //set the unit
         ...
         iter != 0;iter = iter->getNextSibling())
    {
       ...
       if (tag=="Surfaces")         {aSurfaceList = HalfSpaceSurfacesRead(child); } else
       if (tag=="BoundaryBox")      {aBoundaryBox = BoundaryBoxRead(child); }   else
       if (tag=="VolumeSize")       {aVolumeSize = VolumeSizeRead(child) ; } else
       if (tag=="SurfaceArea")      {aSurfaceArea = SurfaceAreaRead(child); } else
       if (tag=="VisualPolyhedron") {aVisPolyhedron = GetPolyhedron(RefRead(child)); }
       else
       {
         G4String error_msg = "Unknown tag in define: "+tag;
         G4Exception("G4GDMLReadDefine::defineRead()", "ReadError",
    ...
    G4HalfSpaceSolid * aSolid = new G4HalfSpaceSolid(aName,aSurfaceList,aBoundaryBox->GetBoxMin(), aBoundaryBox->GetBoxMax(),aVolumeSize,aSurfaceArea);
    aSolid->SetPolyhedron(aVisPolyhedron);
    delete aBoundaryBox; //the solid create one by itself
    return ; //aSolid is already in G4SolidStore
}

//for HalfSpaceBoolean


G4VSolid*  G4GDMLReadSolids::NestedBooleanRead(const xercesc::DOMElement* const Element,const  BooleanOp aOp )
{
    std::vector <G4VSolid *> aSolidList;

    //loop the children element
    for (xercesc::DOMNode* iter = Element->getFirstChild();
         iter != 0;iter = iter->getNextSibling())
    {
       if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE) { continue; }
       ...
       if (tag=="Solid")    { aSolidList.push_back( GetSolid(RefRead(child))); } else
       if (tag=="Union")   { aSolidList.push_back(NestedBooleanRead(child, UNION) ); }   else
       if (tag=="Intersection") { aSolidList.push_back(NestedBooleanRead(child, INTERSECTION) ); } else
       if (tag=="Subtraction")     { aSolidList.push_back(NestedBooleanRead(child, SUBTRACTION) ); }
       else
       {
         G4String error_msg = "Unknown tag in define: "+tag;
         G4Exception("G4GDMLReadDefine::defineRead()", "ReadError",
    ...
    {
    case UNION:
        return G4HalfSpaceBoolean::Union(aSolidList);
        break;
    case INTERSECTION:
        return G4HalfSpaceBoolean::Intersect(aSolidList);
        break;
    case SUBTRACTION:
        return G4HalfSpaceBoolean::Subtract(aSolidList);
        break;
    }
    return NULL;  //never goes here
}

void  G4GDMLReadSolids::HalfSpaceBooleanRead(const xercesc::DOMElement* const Element)
{
    G4String  aName;
    //first read the attribute
    const xercesc::DOMNamedNodeMap* const attributes
          = Element->getAttributes();
    XMLSize_t attributeCount = attributes->getLength();
    for (XMLSize_t attribute_index=0;
         attribute_index<attributeCount; attribute_index++)
    {
       ...
       if (attName=="name") { aName = attValue;}
...
//then read the operation
         ...
         iter != 0;iter = iter->getNextSibling())
    {
       ...
       if (tag=="Union")            {aSolid = NestedBooleanRead(child, UNION) ; }   else
       if (tag=="Intersection")     {aSolid = NestedBooleanRead(child, INTERSECTION) ; } else
       if (tag=="Subtraction")      {aSolid = NestedBooleanRead(child, SUBTRACTION) ; }else
       if (tag=="VolumeSize")       {aVolumeSize = VolumeSizeRead(child) ; } else
       if (tag=="SurfaceArea")      {aSurfaceArea = SurfaceAreaRead(child); }
       else if (tag=="VisualPolyhedron") {aVisPolyhedron = GetPolyhedron(RefRead(child)); }
       else
       {
         G4String error_msg = "Unknown tag in define: "+tag;
         G4Exception("G4GDMLReadDefine::defineRead()", "ReadError",
    ...
    aSolid->SetName(aName); //set the name, for further reference
    static_cast<G4BooleanSolid*> (aSolid)->SetPolyhedron(aVisPolyhedron);
}
