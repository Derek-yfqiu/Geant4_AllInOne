...
class G4GDMLReadDefine : public G4GDMLRead
{
 ...
 public:
   ...
   //qiu
   G4Polyhedron * GetPolyhedron(const G4String &);
 ...
 protected:
   ...
   //qiu
    G4ThreeVector  NodeRead(const xercesc::DOMElement* const);
    //method to read ONE node
    std::vector < G4ThreeVector >  NodesRead(const xercesc::DOMElement* const);
    //method to read the node list
    std::vector < G4int> FacetRead(const xercesc::DOMElement* const);
    //method to read ONE facet
    std::vector < std::vector <G4int> > FacetsRead(const xercesc::DOMElement* const);
    //method to read the facets, defined by three or four integer which refer the node id
    void PolyhedronRead (const xercesc::DOMElement* const);
    //for read the polyhedron,used in visualization of the HalfSpaceSolid
    G4Color ColorRead (const xercesc::DOMElement* const);
 ...
 protected:
   ...
   //qiu
   std::map<G4String,G4Polyhedron*> polyhedronMap; //mapping polyhedron pointer with name
};
