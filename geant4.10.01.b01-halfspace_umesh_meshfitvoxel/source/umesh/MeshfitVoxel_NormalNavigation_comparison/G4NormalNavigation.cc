                                      ...
                                      G4int &blockedReplicaNo)
{
  ...
  //
  //qiu remove tmpoarry to compared with MeshfitVoxel result
//  motherSafety = motherSolid->DistanceToOut(localPoint);
//  ourSafety = motherSafety; // Working isotropic safety
  motherSafety= kInfinity;
  ourSafety = kInfinity;
...
//            G4cout <<"G4Normal Hit element No\t" << sampleNo<< "  with Step\t"<<sampleStep<< G4endl;
  ...
  else
  {
    ...
    {
        //qiu remove temporary for testing
//      G4double motherStep = motherSolid->DistanceToOut(localPoint,
//                                                       localDirection,
//                                                       true,
//                                                       &validExitNormal,
//                                                       &exitNormal);
       G4double motherStep = kInfinity ;
#ifdef G4VERBOSE
      if ( fCheck )
      {
                                           ...
                                           const G4double)
{
  ...
  // Compute mother safety
  //
//qiu  motherSafety = motherSolid->DistanceToOut(localPoint);
//qiu  ourSafety = motherSafety; // Working isotropic safety
  ourSafety = kInfinity;  //for test
  ...
  for ( sampleNo=localNoDaughters-1; sampleNo>=0; sampleNo-- )
  {
    ...
    {
        //qiu for testing
//        G4cout << "New safety "<<sampleSafety  << " \t in element \t"<<sampleNo<<G4endl;
      ourSafety = sampleSafety;
    }
#ifdef G4VERBOSE
    if(fCheck)
