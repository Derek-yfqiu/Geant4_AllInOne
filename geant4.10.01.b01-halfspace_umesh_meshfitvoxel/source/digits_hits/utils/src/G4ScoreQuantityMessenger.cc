...
void G4ScoreQuantityMessenger::SetNewValue(G4UIcommand * command,G4String newVal)
{
      ...
      if(command==qTouchCmd) {
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
              //for following change is for unstrutural mesh,
              //we would like to use the default GetIndex() of G4VPrimitiveScorer
//qiu              G4PSCellCharge3D* ps = new G4PSCellCharge3D(token[0]);
              G4PSCellCharge* ps ;
              if (mesh->GetShape() != unstrMesh)
                  ps = new G4PSCellCharge3D(token[0]);
              else
                  ps = new G4PSCellCharge(token[0]);
              ps->SetUnit(token[1]);
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
            ...
            //qiu
            else if (mesh->GetShape()==unstrMesh) {
                G4PSCellFlux* ps = new G4PSCellFlux(token[0]);
                ps->SetUnit(token[1]);
                mesh->SetPrimitiveScorer(ps);
            }
          }
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
            ...
            //qiu
                ...
                G4PSPassageCellFlux* ps = new G4PSPassageCellFlux(token[0]);
                ps->SetUnit(token[1]);
                mesh->SetPrimitiveScorer(ps);
            }
          }
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSEnergyDeposit3D* ps =new G4PSEnergyDeposit3D(token[0]);
              G4PSEnergyDeposit* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSEnergyDeposit3D(token[0]);
              else
                  ps = new G4PSEnergyDeposit(token[0]);
              ps->SetUnit(token[1]);
              mesh->SetPrimitiveScorer(ps);
          }
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
            ...
            //qiu
                ...
                G4PSDoseDeposit* ps = new G4PSDoseDeposit(token[0]);
                ps->SetUnit(token[1]);
                mesh->SetPrimitiveScorer(ps);
            }
          }
          ...
          if ( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSNofStep3D* ps = new G4PSNofStep3D(token[0]);
              G4PSNofStep* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSNofStep3D(token[0]);
              else
                  ps = new G4PSNofStep(token[0]);
              ps->SetBoundaryFlag(StoB(token[1]));
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qnOfSecondaryCmd) {
          if ( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSNofSecondary3D* ps =new G4PSNofSecondary3D(token[0]);
              G4PSNofSecondary* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSNofSecondary3D(token[0]);
              else
                  ps = new G4PSNofSecondary(token[0]);
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qTrackLengthCmd) {
          if ( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSTrackLength3D* ps = new G4PSTrackLength3D(token[0]);
              G4PSTrackLength* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSTrackLength3D(token[0]);
              else
                  ps = new G4PSTrackLength(token[0]);
          ...
          if( CheckMeshPS(mesh,token[0]) ) {
//qiu              G4PSPassageCellCurrent* ps = new G4PSPassageCellCurrent3D(token[0]);
              G4PSPassageCellCurrent* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSPassageCellCurrent3D(token[0]);
              else
                  ps = new G4PSPassageCellCurrent(token[0]);
              ps->Weighted(StoB(token[1]));
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qPassTrackLengthCmd){
          if( CheckMeshPS(mesh,token[0]) ) {
//qiu              G4PSPassageTrackLength* ps = new G4PSPassageTrackLength3D(token[0]);
              G4PSPassageTrackLength* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSPassageTrackLength3D(token[0]);
              else
                  ps = new G4PSPassageTrackLength(token[0]);
              ps->Weighted(StoB(token[1]));
              ps->SetUnit(token[2]);
              mesh->SetPrimitiveScorer(ps);
          }
          ...
          if( CheckMeshPS(mesh,token[0])) {
//qiu              G4PSFlatSurfaceCurrent3D* ps =
//qiu                new G4PSFlatSurfaceCurrent3D(token[0],StoI(token[1]));
              G4PSFlatSurfaceCurrent* ps;
              if (mesh->GetShape() != unstrMesh)
          ...
          if( CheckMeshPS(mesh, token[0] )) {
//qiu              G4PSFlatSurfaceFlux3D* ps = new G4PSFlatSurfaceFlux3D(token[0],StoI(token[1]));
              G4PSFlatSurfaceFlux* ps;
              if (mesh->GetShape() != unstrMesh)
...
//        }
          ...
          if( CheckMeshPS(mesh,token[0])) {
//qiu              G4PSNofCollision3D* ps =new G4PSNofCollision3D(token[0]);
              G4PSNofCollision* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSNofCollision3D(token[0]);
              else
                  ps = new G4PSNofCollision(token[0]);
              ps->Weighted(StoB(token[1]));
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qPopulationCmd){
          if( CheckMeshPS(mesh,token[0]) ) {
//qiu              G4PSPopulation3D* ps =new G4PSPopulation3D(token[0]);
              G4PSPopulation* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSPopulation3D(token[0]);
              else
                  ps = new G4PSPopulation(token[0]);
              ps->Weighted(StoB(token[1]));
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qTrackCountCmd){
          if( CheckMeshPS(mesh,token[0])) {
//qiu              G4PSTrackCounter3D* ps =new G4PSTrackCounter3D(token[0],StoI(token[1]));
              G4PSTrackCounter* ps;
              if (mesh->GetShape() != unstrMesh)
          ...
          if( CheckMeshPS(mesh,token[0])) {
//qiu              G4PSTermination3D* ps =new G4PSTermination3D(token[0]);
              G4PSTermination* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSTermination3D(token[0]);
              else
                  ps = new G4PSTermination(token[0]);
              ps->Weighted(StoB(token[1]));
              mesh->SetPrimitiveScorer(ps);
          }
          ...
          if( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSMinKinEAtGeneration3D* ps =new G4PSMinKinEAtGeneration3D(token[0]);
              G4PSMinKinEAtGeneration* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSMinKinEAtGeneration3D(token[0]);
              else
                  ps = new G4PSMinKinEAtGeneration(token[0]);
              ps->SetUnit(token[1]);
              mesh->SetPrimitiveScorer(ps);
          }
      } else if(command== qStepCheckerCmd){
          if( CheckMeshPS(mesh,token[0]) ){
//qiu              G4PSStepChecker3D* ps =new G4PSStepChecker3D(token[0]);
              G4PSStepChecker* ps;
              if (mesh->GetShape() != unstrMesh)
                  ps =new G4PSStepChecker3D(token[0]);
              else
                  ps = new G4PSStepChecker(token[0]);
              mesh->SetPrimitiveScorer(ps);
          }
