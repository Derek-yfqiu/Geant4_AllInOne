...
//  meshSphereCreateCmd->SetParameterName("MeshName",false);
  ...
  umeshCreateCmd = new G4UIcmdWithAString("/score/create/unstMesh",this);
  umeshCreateCmd->SetGuidance("Create unstructured scoring mesh.");
  umeshCreateCmd->SetParameterName("MeshName",false);
  //
...
//  mBinCmd->SetGuidance("  P1..Pn-1  :(double) \"paramter from P1 to Pn-1 for division.\"");
  ...
  //unstrucutred mesh command
  umeshDir = new G4UIdirectory("/score/umesh/");
  umeshDir->SetGuidance("   Unstructured Mesh processing commands.");

  meshOpnCmd = new G4UIcmdWithAString("/score/open",this);
  meshOpnCmd->SetGuidance("Open scoring mesh.");
  meshOpnCmd->SetParameterName("MeshName",false);

  umeshOpenCmd = new G4UIcommand("/score/umesh/openUMesh",this);
  umeshOpenCmd->SetGuidance("Open a unstructured mesh file. ");
  umeshOpenCmd->SetGuidance("Parameter <format> speicified the mesh format:");
  umeshOpenCmd->SetGuidance("vtk_legacy: vtk legacy format");
  umeshOpenCmd->SetGuidance("Parameter <filename> speicified the mesh file path and name: e.g. /home/User/mesh.vtk");
  param = new G4UIparameter("format",'s',false);
  umeshOpenCmd->SetParameter(param);
  param = new G4UIparameter("filename",'s',false);
  umeshOpenCmd->SetParameter(param);



  umeshDimFactorCmd = new G4UIcommand("/score/umesh/setFactor",this);
  umeshDimFactorCmd->SetGuidance("Set the dimension factor, this factor will multiply with the mesh coordinates");
  umeshDimFactorCmd->SetGuidance("Therefore mesh  in one dimension unit can convert to mm");
  umeshDimFactorCmd->SetGuidance("Parameter <Factor>  is the conversion factor from mesh unit to mm");
  param = new G4UIparameter("Factor",'d',false);
  param->SetParameterRange("Factor>0");
  umeshDimFactorCmd->SetParameter(param);


  // Draw Scoring result
  drawCmd = new G4UIcommand("/score/draw",this);
  drawCmd->SetGuidance("Draw the scored quantities.");
  drawCmd->SetGuidance("Parameter <proj> specified which projection(s) to be drawn.");
  drawCmd->SetGuidance("In case of Unstructured mesh, the <proj> is not avaiable");
...
//      fSMan->DrawChart(meshName,psName,onFlag,scaleOption);
  ...
  //add unstructured mesh creation
  else if(command==umeshCreateCmd) {
      G4VScoringMesh* currentmesh = fSMan->GetCurrentMesh();
      if ( currentmesh ){
          G4cerr << "ERROR[" << umeshCreateCmd->GetCommandPath()
                 << "] : Mesh <" << currentmesh->GetWorldName()
                 << "> is still open. Close it first. Command ignored." << G4endl;
      }
      else {
          G4VScoringMesh*  mesh = fSMan->FindMesh(newVal);
          if ( !mesh ){
              mesh = new G4ScoringUMesh(newVal);
              fSMan->RegisterScoringMesh(mesh);
          }
          else{
              G4cerr << "ERROR[" << umeshCreateCmd->GetCommandPath()
...
//	  } else
          ...
          else if(command==umeshOpenCmd) {
              G4Tokenizer next(newVal);
              G4String MeshFormat  = next();
              G4String MeshFileName = next();
              mesh->setUMeshFormat(MeshFormat);
              mesh->setUMeshFileName(MeshFileName);
              mesh->openUMeshFile();
//              static_cast <G4ScoringUMesh* >(mesh)->openUMeshFile();
          }
          else if(command==umeshDimFactorCmd) {
              G4double aFactor = StoD(newVal);
              mesh->setDimensionFactor(aFactor);
          }
         } else{
