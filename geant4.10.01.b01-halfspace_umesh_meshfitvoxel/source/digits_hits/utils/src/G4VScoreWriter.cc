...
void G4VScoreWriter::DumpQuantityToFile(const G4String& psName,
  ...
  if (fScoringMesh->GetShape() == unstrMesh)
  {
     WriteUMeshScore(fileName, option, psName);
  }
  else
  {
      // confirm the option
                                             ...
                                             const G4String& option)
{
  ...
  if (fScoringMesh->GetShape() == unstrMesh)
  {
     WriteUMeshScore(fileName, option);
  }
  else
  {
      // confirm the option
...
//for unstructured mesh
void  G4VScoreWriter::WriteUMeshScore(const G4String& fileName, const G4String option, const G4String psName)
{
    if (fileName.isNull() ) {
        G4cerr << " The file name should be specified! "<<endl;
        return;
    }

    //create new writer
    G4UMeshWriter * aWriter = 0;
    if (option.isNull() || option == "vtk_legacy") {
        aWriter = new G4UMeshVTKLegacyWriter;
        //set the data for the writer
        aWriter->setFileName(fileName);
        aWriter->setMeshParser(static_cast <G4ScoringUMesh *>(fScoringMesh)->getUMeshParser());//we know this mesh is a umesh
        std::map<G4String,G4THitsMap<G4double>* > aScoreMap =
                static_cast <std::map<G4String,G4THitsMap<G4double>* > >(fScoringMesh->GetScoreMap()); //we known they are the same type
        aWriter->setFieldData(aScoreMap);
        //form a unit map, and set it
        std::map<G4String,G4THitsMap<G4double>* >::iterator aMapItr;
        std::map<G4String,G4double> aUnitMap;
        for (aMapItr = aScoreMap.begin(); aMapItr != aScoreMap.end(); aMapItr++){
            G4String aPSName = aMapItr->first;
            G4double aUnitValue = fScoringMesh->GetPSUnitValue(aPSName);
            aUnitMap[aPSName] = aUnitValue; //add a new key and value pair
        }
        aWriter->setFieldUnits(aUnitMap);

        //dump the file now
        if (aWriter->dumpToFile(psName))
            G4cout << "Unstructured mesh export is done! "<<endl;
        else
            G4cerr << "Error: dump file failed! "<<endl;
    }
    else {
        G4cerr << "Unsupported format for writing unstructured mesh results! "<<endl;
    }

    if (aWriter != 0) delete aWriter;
    return;
}
